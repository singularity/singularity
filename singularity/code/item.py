...
class ItemType(GenericSpec):
    ...
    spec_data_fields = [
        SpecDataField(
            "is_extra", data_field_name="is_extra", converter=lambda v: bool(int(v))
        ),
    ]
...
def convert_item_types(raw_value):
    try:
        ...
        raise ValueError(
            "Unknown item type %s, please use one of: %s"
            % (raw_value, ", ".join(item_types))
        )
    return item_type
...
def convert_item_qualities(raw_value):
    ...
    if len(raw_value) % 2 == 1:
        raise ValueError(
            "item quality list must have pair elements, got %d (value: %s)"
...
class ItemSpec(buyable.BuyableSpec):
    ...
    spec_data_fields = [
        ...
        SpecDataField(
            "qualities", data_field_name="quality", converter=convert_item_qualities
        ),
        SpecDataField(
            "buildable",
...
class Item(buyable.Buyable):
    ...
    def deserialize_obj(cls, base, obj_data, game_version):
        spec_id = g.convert_internal_id(
            "item", obj_data.get("id", None) or obj_data["spec_id"]
        )
    ...
    def __iadd__(self, other):
        if (
            ...
            total_cost_paid[buyable.labor] = min(
                self.cost_paid[buyable.labor], other.cost_paid[buyable.labor]
            )
