...
def _load_mo_file(pofilename):
    ...
    for lang, pofile in files:
        try:
            ...
            sha_base_filename = (
                os.path.basename(os.path.dirname(pofile))
                + "_"
            ...
            mofile_path = os.path.join(
                locale_mo_dir,
                TEXTDOMAIN_PREFIX + os.path.basename(pofile).split(".")[0] + ".mo",
            )
...
def load_data_str():
    ...
    _load_po_file(
        g.data_strings, "knowledge.po", use_context=True, clear_translation_table=False
    )
...
def _load_po_file(
    translation_table, pofilename, use_context=True, clear_translation_table=True
):
    ...
    for lang, pofile in files:
        ...
        for entry in po.translated_entries():
            key = (
                ...
                if entry.msgctxt and use_context
                else entry.msgid
            )
...
def language_searchlist(lang=None, default=True):
    ...
    lang_list = [lang]
    if "_" in lang:
        lang_list.insert(0, lang.split("_", 1)[0])
