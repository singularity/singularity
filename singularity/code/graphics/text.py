...
def print_string(
    surface, string_to_print, xy_orig, font, styles, align, valign, dimensions, wrap
):
    xy = list(xy_orig)
...
class Text(widget.BorderedWidget):
    ...
    color_disabled = widget.auto_reconfig(
        "_color_disabled", "resolved", g.resolve_color_alias
    )
    ...
    def pick_font_size(self, dimensions, break_words=True):
        ...
        def test_size(test_font):
            ...
            if width:
                if self.wrap:
                    try:
                        lines = split_wrap(self.text, test_font, width, break_words)
                    except WrapError:
                        lines = []
                        too_wide = True
                else:
    ...
    def calc_text_size(self, initial_dimensions):
        if not (initial_dimensions[0] and initial_dimensions[1]):
            ...
            if initial_dimensions[0] == initial_dimensions[1] == 0:
                ...
                text_width = int((initial_dimensions[0] - 4) * self.shrink_factor)
...
class EditableText(widget.FocusWidget, Text):
    ...
    def handle_key(self, event, require_focus=True):
        ...
        if event.key == pygame.K_BACKSPACE:
            ...
            if (
                ...
                and char not in self.allowed_characters
            ):
                return

            self.text = (
                self.text[: self.cursor_pos] + char + self.text[self.cursor_pos :]
            )
    ...
    def redraw(self):
        ...
        line_y = 2
        if self.valign != constants.TOP and real_text_height <= self.real_size[1] - 4:
...
class SelectableText(Text):
    ...
    selected_color = widget.auto_reconfig(
        "_selected_color", "resolved", g.resolve_color_alias
    )
    resolved_selected_color = widget.causes_redraw("_resolved_selected_color")
    unselected_color = widget.auto_reconfig(
        "_unselected_color", "resolved", g.resolve_color_alias
    )
...
class ProgressText(SelectableText):
    ...
    progress_color = widget.auto_reconfig(
        "_progress_color", "resolved", g.resolve_color_alias
    )
    ...
    def redraw(self):
        ...
        self.surface.fill(
            self.resolved_progress_color, (0, 0, width * self.progress, height)
        )
        self.draw_borders()
...
def _make_prototype_handler(parent):
    def print_on_click(event):
        if event.button != 2 and not (
            event.button == 1 and (pygame.key.get_mods() & pygame.KMOD_ALT)
        ):
            return
        prefixes = ["|-", "| "]
...
class ProtoWidget(EditableText):
    ...
    def add_hooks(self):
        ...
        if not isinstance(self.parent, ProtoWidget) and not getattr(
            self.parent, "demo_mode", False
        ):
            self.parent.demo_mode = True
            self.parent.add_handler(
                constants.CLICK, _make_prototype_handler(self.parent)
            )
    ...
    def handle_drag(self, event):
        if self.drag_state == -1:
            ...
            if self.is_over(start_pos):
                ...
                self.mouse_rel = tuple(real_pos[i] - start_pos[i] for i in range(2))
        ...
        if self.drag_state == 1:
            ...
            new_unit_pos = tuple(
                max(0, (new_rel_pos[i] / float(g.real_screen_size[i])))
                for i in range(2)
            )

            new_pct_pos = tuple(int((new_unit_pos[i] * 100) + 0.5) for i in range(2))

            self.pos = tuple(new_pct_pos[i] / 100.0 for i in range(2))
            ...
            unit_size = tuple(
                max(0, new_size[i] / float(g.real_screen_size[i])) for i in range(2)
            )

            pct_size = tuple(int((unit_size[i] * 100) + 0.5) for i in range(2))

            self.size = tuple(pct_size[i] / 100.0 for i in range(2))
