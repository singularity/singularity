...
class Dialog(text.Text):
    ...
    def handle(self, event):
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            if what == old_what and when - old_when < 0.5:
                # Taxicab distance.
                dist = abs(where[0] - old_where[0]) + abs(where[1] - old_where[1])

                if dist < 10:
                    # Add double-click handlers, but keep the click handlers.
                    insort_all(handlers, self.handlers.get(constants.DOUBLECLICK, []))
...
class TextDialog(Dialog):
    def __init__(
        ...
        **kwargs
        ...
        kwargs.setdefault("shrink_factor", 0.88)
...
class YesNoDialog(TextDialog):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self.yes_button = button.ExitDialogButton(
            self,
            (-0.1, -0.99),
            (-0.3, -0.1),
            text=yes_type,
        ...
        self.no_button = button.ExitDialogButton(
            self,
            (-0.9, -0.99),
            (-0.3, -0.1),
            text=no_type,
...
class TextEntryDialog(TextDialog, FocusDialog):
    def __init__(
        ...
        **kwargs
        ...
        self.ok_button = button.FunctionButton(
            self,
            (-0.14, -0.65),
            (-0.30, -0.25),
            autotranslate=True,
...
class ChoiceDescriptionDialog(ChoiceDialog):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        self.description_pane = widget.BorderedWidget(
            self, (-1, 0), (-0.45, -0.85), anchor=constants.TOP_RIGHT
        )

    def make_listbox(self):
        return listbox.UpdateListbox(
            self,
            (0, 0),
            (-0.53, -0.85),
            anchor=constants.TOP_LEFT,
...
class SimpleMenuDialog(Dialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.button_panel = widget.BorderedWidget(
            self,
            (-0.5, -0.5),
            (0.22, 0.43),
            anchor=constants.MID_CENTER,
    ...
    def rebuild(self):
        ...
        height = (0.06 * num_buttons) + 0.01

        self.button_panel.size = (self.width + 0.02, height)

        y_pos = 0.01
        for button in self.buttons:
            button.parent = self.button_panel

            button.pos = (0.01, y_pos)
            button.size = (self.width, 0.05)
            button.text_size = 24
